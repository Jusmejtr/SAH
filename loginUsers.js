...
export const getLoginUsersPath = (installDir) =>
  path.join(installDir, "config", "loginusers.vdf");

export const applyMostRecentUser = (installDir, username) => {
  const file = getLoginUsersPath(installDir);
  ...
  for (const entry of Object.values(users)) {
    ...
    if (isTarget) {
      remembered = entry.RememberPassword === "1";
